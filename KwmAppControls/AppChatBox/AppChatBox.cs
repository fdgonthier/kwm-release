...
namespace kwm.KwmAppControls
{
    ...
    {
        private const String MessageBoard = "      Message board";
        /// <summary>
        ...
        public void OnUserChanged(object caller, EventArgs _args)
        {
            ...
            if (args.ID == Helper.KwmUser.UserID)
            {
                // Do not allow chatting with ourself in the public workspace.
                DoOnAppChatUserChanged(new OnAppChatUserChangedEventArgs(MessageBoard, 0));
            }
            else
            {
                // Chatting with someone in the public workspace.
                DoOnAppChatUserChanged(new OnAppChatUserChangedEventArgs(MessageBoard + " with " + 
                                       Helper.GetUserByID(args.ID).UiSimpleName, args.ID));
            }
        }
        ...
        {
            Logging.Log("FormatChatMessage(" + _userID + ") called.");
            KwsUser u = Helper.GetUserByID(_userID);
            String name;
            Debug.Assert(u != null);
            if (u == null) 
            {
                Logging.Log("Could not locate user.");
                name = "(unknown)";
            }
            else
            {
        ...
        public override KwsAnpEventStatus HandleAnpEvent(AnpMsg msg)
        {
            ...
            if (msg.Type == KAnpType.KANP_EVT_CHAT_MSG)
            {
                ...
                // someone else.
                if (Helper.KwmUser.UserID != userID)
                {
