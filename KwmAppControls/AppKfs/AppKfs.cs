...
namespace kwm.KwmAppControls.AppKfs
{
    ...
    public sealed class AppKfs : KwsApp, ISerializable
    {
        ...
        public override void HandleOnKwmFocusChanged(bool isFocused)
        {
            Logging.Log("HandleOnKwmFocusChanged(" + isFocused + ") called.");
            if (Helper.GetRunLevel() != KwsRunLevel.Online) return;
            if (isFocused) Share.Activate();
            else Share.Deactivate();
        }
        /// <summary>
        ...
        private void OnDownloadEvent(AnpMsg msg)
        {
            ...
            if (!Helper.IsPublicKws() ||
                msg.Elements[2].UInt32 == Helper.KwmUser.UserID)
            {
                return;
            }
