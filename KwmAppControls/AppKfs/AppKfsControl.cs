...
namespace kwm.KwmAppControls.AppKfs
{
    ...
    public partial class AppKfsControl : BaseAppControl
    {
        ...
        /// <summary>
        /// Reference to the KFS toolbar hosted in the main form. Set
        /// by frmMain in InitializeApplications().
        /// </summary>
        private ToolStrip m_kfsToolbar = null;
        ...
        /// <summary>
        /// Set a reference to the main form's KFS toolbar control.
        /// </summary>
        public ToolStrip KfsToolbar
        {
            set { m_kfsToolbar = value; }
        }

        private ToolStripSplitButton BtnNew { get { return m_kfsToolbar.Items["btnNew"] as ToolStripSplitButton; } }
        private ToolStripSplitButton BtnAdd { get { return m_kfsToolbar.Items["btnAdd"] as ToolStripSplitButton; } }
        private ToolStripButton BtnSync { get { return m_kfsToolbar.Items["btnSync"] as ToolStripButton; } }
        private ToolStripButton BtnOffline { get { return m_kfsToolbar.Items["btnViewOffline"] as ToolStripButton; } }
        private ToolStripMenuItem BtnAddFile { get { return BtnAdd.DropDownItems["addFileToolstrip"] as ToolStripMenuItem; } }
        private ToolStripMenuItem BtnAddFolder { get { return BtnAdd.DropDownItems["addFolderToolstrip"] as ToolStripMenuItem; } }
        ...
        /// <summary>
        /// Provide the kfs Toolbar to the control and hook the appropriate 
        /// event handlers.
        /// </summary>
        public void InitializeApp(ToolStrip kfsToolbar)
        {
            m_kfsToolbar = kfsToolbar;
            HookEventHandlers();
        }
        ...
        /// <summary>
        /// Hook the toolbar events to local handlers. Call only once, this 
        /// function does not unhook anything.
        /// </summary>
        private void HookToolbarEvents()
        {
            BtnNew.ButtonClick += btnNew_ButtonClick;
            BtnNew.DropDownClosed += btnNew_DropDownClosed;
            BtnNew.DropDownOpening += btnNew_DropDownOpening;
            BtnNew.DropDownItemClicked += btnNew_DropDownItemClicked;

            BtnAdd.ButtonClick += btnImport_Click;
            BtnAddFile.Click += importFilesToolStrip_Click;
            BtnAddFolder.Click += addFolderToolStrip_Click;

            BtnSync.Click += btnSyncAll_Click;
            BtnOffline.Click += btnViewOfflineFiles_Click;
        }
        ...
        protected override void UpdateControls()
        {
            ...
            BtnOffline.Enabled = CanWorkOffline();
        ...
        private void UpdateUI()
        {
            ...
            try
            {
                ...
                if (GetRunLevel() == KwsRunLevel.Online && SrcApp.Share.LicenseErrorRaised)
                {
                    if (SrcApp.Helper.IsCreatorKwmUser)
                    {
                        SrcApp.Helper.PostGuiExecRequest(new LicenseRestrictionGer(
                            "Your license quota has been exceeded. To add new files, " + 
                            "you must either delete existing files or upgrade your account.", true));
                    }
                    else
                    {
                        SrcApp.Helper.PostGuiExecRequest(new LicenseRestrictionGer(
                            "The " + Base.GetKwsString() + " creator, " + SrcApp.Helper.KwsCreator.UiFullName + ", has exceeded his storage limit. Please contact him to resolve the situation.", false));
                    }
                    
                    SrcApp.Share.ClearLicenseError();
        ...
        private void SetLvColumnWidth()
        {
            try
            {
                ...
                lvFileList.Columns[3].Width = Misc.ApplicationSettings.KfsStatusColumnWidth; 
                lvFileList.Columns[4].Width = Misc.ApplicationSettings.KfsModDateColumnWidth;
                lvFileList.Columns[5].Width = Misc.ApplicationSettings.KfsModByColumnWidth;
        ...
        private void HandleListviewSaveAsMenu(object sender, EventArgs e)
        {
            try
            {
                ...
                if (res == DialogResult.OK)
                {
                    ...
                    String errorMessage = "";
                    if (Misc.CopyFile(SrcApp.Share.MakeAbsolute(GetSelectedItem().Path), dlg.FileName, true, true, false, false, false, ref errorMessage) == ShellLib.ShellFileOperation.CopyFileResult.Failed)
                        throw new Exception("could not save file: " + errorMessage);
                }   
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Shows or hide the background image, depending on the
        /// content of the share and if we are in a public workspace
        /// or not.
        /// </summary>
        private void UpdateBackgroundImage()
        {
            if (!SrcApp.Helper.IsPublicKws() &&
                SrcApp.Share.StatusView.Root.ChildTree.Count == 0 &&
                !m_forceDisableUI)
            {
                lvFileList.BackgroundImage = KwmAppControls.Properties.Resources.drag_files_here; 
            }

            else
            {
                lvFileList.BackgroundImage = null;
            }
        }
        ...
        private void lvFileList_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            // If exitGateReason is non-empty, exit the gate in the finally block.
            String exitGateReason = "";
            try
            {
                ...
                else if (e.Control && e.KeyCode == Keys.C)
                {
                    ...
                    if (c != m_uiUpdateCounter)
                    {
                        exitGateReason = s + " refused, share has been modified.";
                        return;
                    }
            ...
            {
                if (exitGateReason != "") Gate.GateExit(exitGateReason);
            }
        }
        ...
        private void btnNew_DropDownOpening(object sender, EventArgs e)
        {
            try
            {
                ...
                // Populate the content on-demand.
                CreateNewFileButton();
            }
            catch (Exception ex)
            {
