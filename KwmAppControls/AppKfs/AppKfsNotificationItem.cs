...
namespace kwm.KwmAppControls
{
    ...
    public class KfsNotification
    {
        ...
        private void SeparateServerOpByType(SortedDictionary<UInt64, List<KfsServerOp>> seenPhase1Ops)
        {
            foreach (KfsServerOp op in m_incomingServerOps)
            {
                ...
                {
                    KfsCreateServerOp o = op as KfsCreateServerOp;
                    // Ignore directory creation in public workspace.
                    if (o.IsFile || !o.Share.App.Helper.IsPublicKws())
                        m_createOps.Add(op as KfsCreateServerOp);
                }

                else if (op is KfsDeleteServerOp)
                {
                    IAppHelper helper = op.Share.App.Helper;
                    KwsUser source = helper.GetUserByID(op.UserID);

                    // Better safe than sorry.
                    if (source == null) continue;

                    // Do not report the operation if it's coming from the Root user in 
                    // the public workspace. That particular operation occurs when
                    // purging expired attachment management files.
                    if (!(helper.IsPublicKws() && source.PrivLevel == UserPrivLevel.Root))
    ...
    public class AppKfsCreateItem : AppKfsNotificationItem
    {
        ...
        protected override string GetMessage()
        {
            ...
            String who = m_helper.GetUserByID(FirstOp.UserID).UiSimpleName;
    ...
    public class KfsFileDownloadedNotificationItem : NotificationItem
    {
        ...
        public override string EventText
        {
            get
            {
                return m_helper.GetUserByID(m_downloaderUID).UiSimpleName + " is downloading " + m_fileName;
            }
        }
