...
namespace kwm.KwmAppControls.AppKfs
{
    ...
    public class KfsGate
    {
        ...
        public void GateExit(string _reason)
        {
            ...
            // Notify the listeners if required.
            if (EntryCount == 0)
            {
        ...
        public void AddExternalFiles(String destRelPath, string[] toAdd, bool addDir, GateConfirmActions defaultActions)
        {
            ...
            try
            {
                ...
                // If something in srcArray is a parent folder of destRelPath, deny the Add.
                foreach (String p in srcArray)
                {
                    if (Directory.Exists(p) &&
                        KfsPath.GetWindowsFilePath(Share.MakeAbsolute(destRelPath), true).StartsWith(p, StringComparison.OrdinalIgnoreCase))
                    {
                ...
                String errorMessage = "";
                ShellLib.ShellFileOperation.CopyFileResult res = Misc.CopyFiles(copySrc, copyDst, false, true, false, false, false, true, ref errorMessage);
                if (res == ShellLib.ShellFileOperation.CopyFileResult.Aborted) throw new GateCancelledException();
                else if (res == ShellLib.ShellFileOperation.CopyFileResult.Failed) throw new Exception(errorMessage);

                // Add the files and directories.            
                AddFilesCommon(fileSet, dirSet, 0);
            }

            catch (Exception ex)
            {
                HandleExceptionInGate(ex);
            }
        ...
        public void AddInternalFiles(List<String> pathArray, ulong emailID)
        {
            ...
            try
            {
                ...
                // Add the files and directories.
                AddFilesCommon(fileSet, dirSet, emailID);
            }
        ...
        public void Paste(String destRelPath)
        {
            ...
            try
            {
                ...
                String errorMessage = "";
                if (Misc.CopyFiles(Source, Dest, false, false, autoRename, false, false, true, ref errorMessage) == ShellLib.ShellFileOperation.CopyFileResult.Failed)
        ...
        public void ResolveFileConflict(List<string> pathArray, bool overwriteLocalChanges)
        {
            ...
            try
            {
                foreach (String s in pathArray)
                {
                    ...
                    {
                        Share.UploadManager.QueueUpload(sp.Path, 0);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void MovePath(List<String> pathArray, String dstRelPath)
        {
            ...
            try
            {
                ...
                foreach (String path in pathArray)
                {
                    ...
                    if (dstRelPath.StartsWith(path, StringComparison.OrdinalIgnoreCase))
        ...
        public void RenamePath(string srcPath, string newName)
        {
            ...
            try
            {
                ...
                // Make sure the destination does not exist or it is the same file.
                String dstPath = dirPath + newName;
                GatePathType dstType = GetInternalPathType(dstPath);
                if (!KfsPath.Eq(KfsPath.BaseName(srcPath), newName) && dstType != GatePathType.None)
                ...
                if (srcObject == null)
                {
                    ...
                    KfsPath.MovePath(!fileFlag, Share.MakeAbsolute(srcPath), Share.MakeAbsolute(dstPath));
                }
        ...
        private void AddFilesCommon(SortedSet<String> fileSet, SortedSet<String> dirSet, ulong emailID)
        {
            KfsPhase1Payload payload = new KfsPhase1Payload();
            ...
            try
            {
                foreach (String path in fileSet)
                {
                    ...
                    KfsFileUpload upload = Share.UploadManager.QueueUpload(path, emailID);
            ...
            foreach (String srcName in srcArray)
            {
                ...
                else
                {
                    ...
                    if (srcObject == null)
                    {
                        ...
                        // Move the file or directory locally.
                        KfsPath.MovePath(!fileFlag, srcFullPath, dstFullPath);
                    }
        ...
        /// </summary>
        private void HandleExceptionInGate(Exception ex)
        {
            String dummy = "";
            ...
            {
                Logging.Log("GateCancelledException trapped. Ignoring.");
            }
