...
namespace kwm.KwmAppControls.AppKfs
{
    ...
    public class KfsMoveServerOp : KfsServerOp
    {
        ...
        public override void Validate()
        {
            ...
            Assert(!d.ChildTree.ContainsKey(DestName) || (p == d && KfsPath.Eq(o.Name, DestName)),
        ...
        protected override KfsApplyOpRequest ApplyOnFileSystemInternal(ref bool fsModifiedFlag)
        {
            ...
            {
                // The destination already exists and it is not the source 
                // (not a case-change-only rename).
                if (!KfsPath.Eq(srcFullPath, destFullPath) &&
                    (File.Exists(destFullPath) || Directory.Exists(destFullPath)))
                ...
                // Perform the move.
                KfsPath.MovePath(!IsFile, srcFullPath, destFullPath);
            }
