...
namespace kwm.KwmAppControls.AppKfs
{
    ...
    public class KfsUploadManager
    {
        ...
        /// </summary>
        public KfsFileUpload QueueUpload(String path, ulong emailID)
        {
            ...
            // Link or copy the file in the upload directory.
            ...
            try
            {
                if (!Syscalls.CreateHardLink(uploadPath, fullPath, IntPtr.Zero))
                    throw new Exception(Syscalls.GetLastErrorStringMessage());
            }

            catch (Exception ex)
            {
                Logging.Log(1, "Could not create hard link: " + ex.Message);
                Base.SafeCopy(fullPath, uploadPath, true);
            }

            // Get the file size.
            UInt64 size = KfsPath.GetFileSize(uploadPath);

            // Queue the upload.
            KfsFileUpload u = new KfsFileUpload(Share, orderID, trackedInode, trackedPath,
                                                path, uploadPath, size, updateCommitID, emailID);
        ...
        /// go idle. 'expReason' is null if the thread has been cancelled.
        /// </summary>
        private void CancelOnError(Exception expReason)
        {
            ...
            foreach (KfsFileUpload f in tree.Values)
            {
                ...
                {
                    // Make sure we do not end up here if the thread is cancelled 
                    if (expReason != null) ReportError(expReason.Message, f);
                    RemoveUpload(f);
                }
            }

            // Report a license error in the appropriate case.
            if (expReason != null && expReason.GetType() == typeof(LicenseRestrictionException))
                Share.onLicenseError();
    ...
    public class KfsFileUpload : KfsFileTransfer
    {
        ...
        /// <summary>
        /// Email ID the file upload is associated to.
        /// </summary>
        public ulong EmailID;
        ...
        public void UpdateLastFullPath()
        {
            ...
            if (o != null)
            {
                ...
                if (!KfsPath.Eq(LastFullPath, newPath))
                {
    ...
    public class KfsUploadThread : KfsTransferThread
    {
        ...
        private void PerformPhase2()
        {
            ...
            /* Another Microsoft moron was allowed near a keyboard. See
            ...
            while (UploadIndex != FileArray.Length)
            {
                ...
                CreateUploadMsg(md5Hasher, out m, out CompletedArray);
                ...
                if (m != null)
                {
                    ...
                    m = GetAnpMsg();
                    if (m.Type == KAnpType.KANP_RES_FAIL)
                    {
                        if (m.Elements[0].UInt32 == KAnpType.KANP_RES_FAIL_RESOURCE_QUOTA)
                            throw new LicenseRestrictionException(m.Elements[1].String, m.Elements[2].UInt32);
                        else
                            throw new Exception(m.Elements[1].String);
                    }
        ...
        protected override void OnCompletion()
        {
            ...
            if (Status == WorkerStatus.Cancelled)
                ...
                Share.UploadManager.OnBatchCompleted(false, FailException, 0);
                ...
                Share.UploadManager.OnBatchCompleted(false, new Exception("No file could be uploaded"), 0);
    ...
    {
        public KfsUploadBatchFile(UInt64 orderID, String transferPath)
