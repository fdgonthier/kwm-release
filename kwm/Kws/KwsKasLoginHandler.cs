...
namespace kwm
{
    ...
    public class KwsKasLoginHandler
    {
        ...
        {
            if (code == KAnpType.KANP_KWS_LOGIN_OK) return KwsLoginResult.Accepted;
            if (code == KAnpType.KANP_KWS_LOGIN_BAD_PWD_OR_TICKET) return KwsLoginResult.BadSecurityCreds;
            if (code == KAnpType.KANP_KWS_LOGIN_OOS) return KwsLoginResult.OOS;
            if (code == KAnpType.KANP_KWS_LOGIN_BAD_KWS_ID) return KwsLoginResult.BadKwsID;
            if (code == KAnpType.KANP_KWS_LOGIN_BAD_EMAIL_ID) return KwsLoginResult.BadEmailID;
            if (code == KAnpType.KANP_KWS_LOGIN_DELETED_KWS) return KwsLoginResult.DeletedKws;
            if (code == KAnpType.KANP_KWS_LOGIN_ACCOUNT_LOCKED) return KwsLoginResult.AccountLocked;
            if (code == KAnpType.KANP_KWS_LOGIN_BANNED) return KwsLoginResult.Banned;
        ...
        private void SendLoginCommand()
        {
            ...
            {
                msg.AddUInt32(Convert.ToUInt32(m_kws.Sm.GetCurrentTask() == KwsTask.DeleteOnServer));
            }
