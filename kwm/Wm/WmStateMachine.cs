...
namespace kwm
{
    ...
    public class WmStateMachine
    {
        ...
        /// </summary>
        private void ProcessKwsToRemove()
        {
            // Fast track.
            if (m_wm.KwsRemoveTree.Count == 0) return;

            SortedDictionary<UInt64, Workspace> tree = new SortedDictionary<UInt64, Workspace>(m_wm.KwsRemoveTree);
            foreach (Workspace kws in tree.Values)
                if (m_wm.KwsRemoveTree.ContainsKey(kws.InternalID)) ProcessOneKwsToRemove(kws);
        }
