...
namespace kwm
{
    ...
    public enum KwsAction
    {
        ...
        /// </summary>
        [Base.Description("change the Teambox type")]
        ChangeSecureFlag,

        /// <summary>
        /// Set or unset the freeze flag.
        /// </summary>
        [Base.Description("change the moderated attribute")]
        ChangeFreezeFlag,

        /// <summary>
        /// Set or unset the deep freeze flag.
        /// </summary>
        [Base.Description("change the lock attribute")]
        ChangeDeepFreezeFlag,

        /// <summary>
        /// Set or unset the KFS option to keep the deleted files
        /// on the server instead of permanently deleting them.
        /// </summary>
        [Base.Description("change the preserve deleted files attribute")]
        ChangeThinKfsFlag,

        /// <summary>
        /// Change the workspace name.
        /// </summary>
        [Base.Description("change Teambox name")]
        Rename,

        /// <summary>
        /// Show the workspace properties dialog.
        /// </summary>
        ShowProperties
    }
    ...
    public enum KwsUserAction
    {
        ...
        /// </summary>
        [Base.Description("reset password")]
        ResetPassword,

        /// <summary>
        /// Lock the user account.
        /// </summary>
        [Base.Description("change locked status")]
        ChangeLockFlag,

        /// <summary>
        /// Ban the user from the workspace.
        /// </summary>
        [Base.Description("delete user")]
        Ban,

        /// <summary>
        /// Change the name of the user. Any user can set its own name, except when
        /// an Administrator has set the user's Administrative name.
        /// </summary>
        [Base.Description("change user name")]
        SetName,

        /// <summary>
        /// Change the administrator flag of the user.
        /// </summary>
        [Base.Description("change user role")]
        ChangeAdminFlag,

        /// <summary>
        /// Change the manager flag of the user.
        /// </summary>
        [Base.Description("change user role")]
        ChangeManagerFlag
    }
    ...
    /// <summary>
    /// This is used to return the list of the most popular email addresses
    /// in the KWM in GetKwsAddressSet
    /// </summary>
    public class FrequencyObject<T> : IComparable
    {
        ...
        {
            // FIXME: Bad CompareTo protocol.
            return Frequency.CompareTo(((FrequencyObject<T>)obj).Frequency);
        }
    ...
    /// <summary>
    /// Represent the privilege information about a workspace action.
    /// </summary>
    public class KwsActionPriv
    {
        /// <summary>
        /// Minor number of the procotol in which the command was introduced.
        /// </summary>
        public UInt32 Minor;
        ...
        /// </summary>
        public UserPrivLevel Level;

        /// <summary>
        /// True if the action can be performed on a public workspace.
        /// </summary>
        public bool PublicFlag;

        public KwsActionPriv(UInt32 minor, UserPrivLevel level, bool publicFlag)
        {
            Minor = minor;
            Level = level;
    ...
    /// <summary>
    /// Represent the privilege information about a user action.
    /// </summary>
    public class KwsUserActionPriv : KwsActionPriv
    {
        /// <summary>
        /// True if the action can be applied on self.
        /// </summary>
        public bool SelfOKFlag;

        /// <summary>
        /// True if the action can be applied on a banned user.
        /// </summary>
        public bool BanOKFlag;

        public KwsUserActionPriv(UInt32 minor, UserPrivLevel level, bool selfOKFlag, bool banOKFlag)
            : base(minor, level, false)
        {
            SelfOKFlag = selfOKFlag;
            BanOKFlag = banOKFlag;
        }
    }
    ...
    public class WmUiBroker : IUiBroker
    {
        ...
        /// UI, if any.
        /// </summary>
        private WmUiRefreshGer m_uiRefreshGer = null;

        /// <summary>
        /// Tree mapping workspace actions to their privilege information.
        /// </summary>
        public SortedDictionary<KwsAction, KwsActionPriv> KwsActionPrivTree = new SortedDictionary<KwsAction, KwsActionPriv>();

        /// <summary>
        /// Tree mapping workspace user actions to their privilege information.
        /// </summary>
        public SortedDictionary<KwsUserAction, KwsUserActionPriv> KwsUserActionPrivTree = new SortedDictionary<KwsUserAction, KwsUserActionPriv>();
        ...
        public WmUiBroker(WorkspaceManager wm)
        {
            ...
            DeclareKwsAction(KwsAction.DeleteFromServer, 4, UserPrivLevel.Admin, true);
            DeclareKwsAction(KwsAction.ChangeSecureFlag, 4, UserPrivLevel.Admin, false);
            DeclareKwsAction(KwsAction.ChangeFreezeFlag, 4, UserPrivLevel.Admin, false);
            DeclareKwsAction(KwsAction.ChangeDeepFreezeFlag, 4, UserPrivLevel.Root, true);
            DeclareKwsAction(KwsAction.ChangeThinKfsFlag, 4, UserPrivLevel.Admin, false);
            DeclareKwsAction(KwsAction.Rename, 4, UserPrivLevel.Admin, false);

            DeclareKwsUserAction(KwsUserAction.ResendInvitation, 3, UserPrivLevel.Manager, true, true);
            DeclareKwsUserAction(KwsUserAction.ResetPassword, 3, UserPrivLevel.Manager, true, true);
            DeclareKwsUserAction(KwsUserAction.ChangeLockFlag, 4, UserPrivLevel.Manager, false, false);
            DeclareKwsUserAction(KwsUserAction.Ban, 4, UserPrivLevel.Manager, false, false);
            DeclareKwsUserAction(KwsUserAction.SetName, 4, UserPrivLevel.Manager, true, true);
            DeclareKwsUserAction(KwsUserAction.ChangeAdminFlag, 4, UserPrivLevel.Root, false, false);
            DeclareKwsUserAction(KwsUserAction.ChangeManagerFlag, 4, UserPrivLevel.Admin, false, false);
        }

        /// <summary>
        /// Add the specified workspace action to KwsActionPrivTree. Set the
        /// public flag if the action can be performed on the public workspace.
        /// </summary>
        private void DeclareKwsAction(KwsAction action, UInt32 minor, UserPrivLevel level, bool publicFlag)
        {
            KwsActionPrivTree[action] = new KwsActionPriv(minor, level, publicFlag);
        }

        /// <summary>
        /// Add the specified user action to KwsUserActionPrivTree.
        /// </summary>
        private void DeclareKwsUserAction(KwsUserAction action, UInt32 minor, UserPrivLevel level, bool selfOKFlag, bool banOKFlag)
        {
            KwsUserActionPrivTree[action] = new KwsUserActionPriv(minor, level, selfOKFlag, banOKFlag);
        }
        ...
        /// <summary>
        /// Set the status bar informations. Used by the static method in Misc,
        /// which is the prefered way of setting these informations.
        /// </summary>
        public void SetStatusbar(StatusBarIcon icon, String message)
        {
            m_mainForm.StatusBarManager.PostStatus(icon, message);
        }
        ...
        /// <summary>
        /// Helper method to check the 'online' status and the 'public' status
        /// of the workspace as required, as well as the protocol minor version.
        /// DenyReason is always set.
        /// </summary>
        private bool CanPerformOnlineKwsAction(KwsActionPriv priv, Workspace kws, KwsUser self, ref String denyReason)
        {
            if (!kws.IsOnlineCapable())
            {
                denyReason = "The " + Base.GetKwsString() + " is not connected.";
                return false;
            }

            // Deny the operation if it cannot be done on the public workspace unless
            // we are root. In this case, allow the action anyway.
            if (kws.IsPublicKws() && !priv.PublicFlag && self.PrivLevel != UserPrivLevel.Root)
            {
                denyReason = "Cannot perform this action in the Attachment Management " + Base.GetKwsString() + ".";
                return false;
            }

            if (kws.Kas.MinorVersion < priv.Minor)
            {
                denyReason = "The server does not support this action.";
                return false;
            }
        ...
        /// <summary>
        /// Return true and set denyReason to empty string if the user is 
        /// allowed to perform the requested action on the given workspace.
        /// Otherwise, return false and set denyReason to a user-friendly 
        /// string.
        /// </summary>
        public bool CanPerformKwsAction(KwsAction action, Workspace kws)
        {
            String unused = "";
            return CanPerformKwsAction(action, kws, ref unused);
        }

        public bool CanPerformKwsAction(KwsAction action, Workspace kws, ref String denyReason)
        {
            // Process non-online commands.
            switch (action)
            {
                case KwsAction.ShowProperties:
                    {
                        if (kws.IsPublicKws())
                        {
                            denyReason = "No properties are available for this special " + Base.GetKwsString() + ".";
                            return false;
                        }
            ...
            switch (action)
            {
                ...
                case KwsAction.DeleteFromServer:
                    {
                    denyReason = "This action cannot be performed at the moment.";
                    if (action == KwsAction.Connect && !kws.Sm.CanWorkOnline()) return false;
                    if (action == KwsAction.Disconnect && !kws.Sm.CanWorkOffline()) return false;
                    if (action == KwsAction.Stop && !kws.Sm.CanStop()) return false;
                    if (action == KwsAction.Export && !kws.Sm.CanExport()) return false;
                    if (action == KwsAction.Rebuild && !kws.Sm.CanRebuild()) return false;
                    if (action == KwsAction.RemoveFromList && !kws.Sm.CanRemove()) return false;
                    if (action == KwsAction.DeleteFromServer && !kws.Sm.CanDeleteOnServer()) return false;
                    break;
                    }
            }

            // Validate online actions.
            switch (action)
            {
                case KwsAction.DeleteFromServer:
                case KwsAction.ChangeSecureFlag:
                case KwsAction.ChangeFreezeFlag:
                case KwsAction.ChangeDeepFreezeFlag:
                case KwsAction.ChangeThinKfsFlag:
                case KwsAction.Rename:
                    {
                        KwsActionPriv priv = KwsActionPrivTree[action];
                        KwsUser self = kws.KwmUser;
                        
                        if (!CanPerformOnlineKwsAction(priv, kws, self, ref denyReason)) return false;
                        
                        if (self.PrivLevel < priv.Level)
                        {
                            denyReason = "Insufficient permission.";
                            return false;
                        }
        ...
        /// <summary>
        /// Return true and set denyReason to empty string if the user is 
        /// allowed to perform the requested action on the given user in 
        /// the given workspace. Otherwise, return false and set denyReason
        /// to a user-friendly string.
        /// </summary>
        public bool CanPerformUserAction(KwsUserAction action, Workspace kws, KwsUser target, ref String denyReason)
        {
            // Process non-online commands.
            switch (action)
            {
                case KwsUserAction.CopyEmailAddr:
                case KwsUserAction.ShowProperties:
                    {
                        denyReason = "";
                        return true;
                    }
            ...
            KwsUserActionPriv priv = KwsUserActionPrivTree[action];
            KwsUser self = kws.KwmUser;
            bool selfFlag = self.UserID == target.UserID;

            // This is the same algorithm as the one used by the KCD. Check the
            // KCD documentation for details.
            do
            {
                if (priv.SelfOKFlag && selfFlag) break;
                if (!priv.SelfOKFlag && selfFlag)
                {
                    denyReason = "Cannot apply this action on yourself.";
                    return false;
                }

                if (!priv.BanOKFlag && target.BanFlag)
                {
                    denyReason = "Cannot apply this action on a banned user.";
                    return false;
                }

                if (self.PrivLevel < target.PrivLevel || self.PrivLevel < priv.Level)
                {
                    denyReason = "Insufficient permission.";
                    return false;
                }
            ...
            // A normal user cannot change his name if he has an Admin name 
            // set.
            if (action == KwsUserAction.SetName && selfFlag && !self.ManagerFlag && self.AdminName != "")
            {
        ...
        public void RequestSelectUser(Workspace kws, UInt32 userID, bool updateUIFlag)
        {
            ...
            KwsUser user = kws.CoreData.UserInfo.GetNonVirtualUserByID(userID);
            if (user == null) return;
        ...
        /// <summary>
        /// Show the workspace user properties form and apply the changes 
        /// as required.
        /// </summary>
        public void RequestChangeUserProperties()
        {    
            Workspace kws = Browser.SelectedKws;
            KwsUser user = GetSelectedUser();

            if (kws == null || user == null)
            {
                Misc.KwmTellUser("This user is no longer available.");
                return;
            }

            frmUserProperties prop = new frmUserProperties(this);
            Misc.OnUiEntry();
            DialogResult res = prop.ShowDialog();
            Misc.OnUiExit();
            if (res != DialogResult.OK) return;

            // Determine which properties were modified so we send the right
            // property change commands.
            List<SetPropertyCommand> commands = new List<SetPropertyCommand>();

            if (prop.InitialUserName != prop.FinalUserName)
            {
                SetUserPropertyCommand c = new SetUserPropertyCommand(kws, user.UserID, KwsUserAction.SetName);
                c.Cmd.AddString(prop.FinalUserName);
                commands.Add(c);
            }

            if (prop.InitialRole != prop.FinalRole)
            {
                if (prop.FinalRole == 0)
                {
                    ChangeUserFlag(commands, kws, user, user.AdminFlag, true, KwsUserAction.ChangeAdminFlag);
                }

                else if (prop.FinalRole == 1)
                {
                    ChangeUserFlag(commands, kws, user, user.AdminFlag, false, KwsUserAction.ChangeAdminFlag);
                    ChangeUserFlag(commands, kws, user, user.ManagerFlag, true, KwsUserAction.ChangeManagerFlag);
                }

                else
                {
                    ChangeUserFlag(commands, kws, user, user.ManagerFlag, false, KwsUserAction.ChangeManagerFlag);
                }
            }

            ChangeUserFlag(commands, kws, user, prop.InitialLockedAccountChecked, 
                           prop.FinalLockedAccountChecked, KwsUserAction.ChangeLockFlag);

            if (commands.Count > 0) SetKwsProperties(kws, commands);
        }

        private void ChangeUserFlag(List<SetPropertyCommand> commands, Workspace kws, KwsUser targetUser, bool initial, bool final, KwsUserAction action)
        {
            if (initial == final) return;
            SetUserPropertyCommand c = new SetUserPropertyCommand(kws, targetUser.UserID, action);
            c.Cmd.AddUInt32(Convert.ToUInt32(final));
            commands.Add(c);
        }

        /// <summary>
        /// Show the workspace properties form and apply the changes as 
        /// required.
        /// </summary>
        public void RequestChangeKwsProperties()
        {
            Workspace kws = Browser.SelectedKws;

            if (kws == null)
            {
                Misc.KwmTellUser("This workspace is no longer available.");
                return;
            }

            frmKwsProperties prop = new frmKwsProperties(this);
            Misc.OnUiEntry();
            DialogResult res = prop.ShowDialog();
            Misc.OnUiExit();
            if (res != DialogResult.OK) return;
            ...
            if (prop.InitialKwsName != prop.FinalKwsName)
            {
                SetKwsPropertyCommand c = new SetKwsPropertyCommand(kws, KwsAction.Rename);
                c.Cmd.AddString(prop.FinalKwsName);
                commands.Add(c);                
            }

            ChangeWorkspaceFlag(commands, kws, prop.InitialFreezeFlag, prop.FinalFreezeFlag, KwsAction.ChangeFreezeFlag);
            ChangeWorkspaceFlag(commands, kws, prop.InitialDeepFreezeFlag, prop.FinalDeepFreezeFlag, KwsAction.ChangeDeepFreezeFlag);
            ChangeWorkspaceFlag(commands, kws, prop.InitialThinKfsFlag, prop.FinalThinKfsFlag, KwsAction.ChangeThinKfsFlag);
            ChangeWorkspaceFlag(commands, kws, prop.InitialKwsType == 2, prop.FinalKwsType == 2, KwsAction.ChangeSecureFlag);

            if (commands.Count > 0) SetKwsProperties(kws, commands);
        }

        private void ChangeWorkspaceFlag(List<SetPropertyCommand> commands, Workspace kws, bool initial, bool final, KwsAction action)
        {
            if (initial == final) return;
            SetKwsPropertyCommand c = new SetKwsPropertyCommand(kws, action);
            c.Cmd.AddUInt32(Convert.ToUInt32(final));
            commands.Add(c);
        }

        public void RequestBanUser()
        {
            Workspace kws = Browser.SelectedKws;
            KwsUser user = GetSelectedUser();
            if (kws == null || user == null) return;

            DialogResult res = Misc.KwmTellUser("Are you sure you want to remove " + user.UiFullName + " from this " + Base.GetKwsString() + "?", "Confirm user delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (res != DialogResult.Yes) return;

            SetUserPropertyCommand c = new SetUserPropertyCommand(kws, user.UserID, KwsUserAction.Ban);
            c.Cmd.AddUInt32(1);
            SetKwsProperty(kws, c);
        }
        ...
        public void RequestSetUserPassword()
        {
            ...
            SetUserPropertyCommand c = new SetUserPropertyCommand(kws, user.UserID, KwsUserAction.ResetPassword);
            c.Cmd.AddString(f.NewPassword);
            SetKwsProperty(kws, c);
        }

        /// <summary>
        /// Rename the given workspace. The name and permissions have already 
        /// been validated.
        /// </summary>
        public void RequestRenameKws(Workspace kws, String newName)
        {
            Debug.Assert(Base.IsValidKwsName(newName));
            SetKwsPropertyCommand c = new SetKwsPropertyCommand(kws, KwsAction.Rename);
            c.Cmd.AddString(newName.Trim());
            SetKwsProperty(kws, c);
        }

        /// <summary>
        /// Execute the specified workspace property change command.
        /// </summary>
        public void SetKwsProperty(Workspace kws, SetPropertyCommand command)
        {
            List<SetPropertyCommand> l = new List<SetPropertyCommand>();
            l.Add(command);
            SetKwsProperties(kws, l);
        }

        /// <summary>
        /// Execute the specified workspace property change commands.
        /// </summary>
        public void SetKwsProperties(Workspace kws, List<SetPropertyCommand> commands)
        {
            (new SetPropertyOp(m_wm, commands)).StartOp(kws);
        }
        ...
        /// </summary>
        public KwsCoreOpRes RequestInviteToKws(Workspace kws, String invitees, bool skipFirstPageFlag)
        {
            try
            {
                Misc.OnUiEntry();
                ...
                {
                    Misc.KwmTellUser("The email address is not in a correct format. Please verify you entered the right email address and try again.", MessageBoxIcon.Error);
                    return KwsCoreOpRes.None;
                }
        ...
        /// <summary>
        /// Prompt for a user confirmation. If the user is allowed to remove
        /// the workspace from the server, the dialog allows the user to switch
        /// to a local deletion. If the user is not allowed th remove the
        /// workspace from the server, he is warned that he will only be able
        /// to remove it locally.
        /// </summary>
        public void RequestGenericDeleteKws(Workspace kws)
        {
            frmDeleteKwsConfDlg f = new frmDeleteKwsConfDlg(this);
            Misc.OnUiEntry();
            DialogResult res = f.ShowDialog();
            Misc.OnUiExit();
            if (res != DialogResult.OK) return;

            if (f.DeleteOnServerFlag)
                RequestDeleteKwsFromServer(kws, false);
            else
                RequestRemoveKwsFromList(kws, false);
        }

        /// <summary>
        /// Return the KwsAction that should be taken when a generic delete
        /// is triggered by the user. A generic delete is when the user uses 
        /// the Delete key or the Delete item in the root context menu of
        /// a workspace.
        /// </summary>
        public KwsAction GetGenDeleteAction(Workspace kws)
        {
            if (CanPerformKwsAction(KwsAction.DeleteFromServer, kws))
                return KwsAction.DeleteFromServer;
            else
                return KwsAction.RemoveFromList;
        }

        public void RequestDeleteKwsFromServer(Workspace kws, bool askConfirmationFlag)
        {
            if (askConfirmationFlag)
            {
                String msg = "You are about to permanently remove this " + Base.GetKwsString() + 
                    " from the server. This action is irreversible: all the data will be lost." +
            ...
            kws.DeleteFromServer();
        }

        /// <summary>
        /// Request the deletion of the workspace specified.
        /// </summary>
        public void RequestRemoveKwsFromList(Workspace kws, bool askConfirmationFlag)
        {
            if (askConfirmationFlag)
            {
            ...
            kws.Sm.RequestTaskSwitch(KwsTask.Remove);
        }
        ...
        public void RequestDeleteFolder(KwsBrowserFolderNode folder)
        {
            ...
            String msg = "Are you sure you want to remove the folder " + folder.Name + " and all the " + Base.GetKwsesString() + " it contains?";
            ...
            // Move the workspace to the primary folder and remove them.
            foreach (KwsBrowserKwsNode kwsNode in kwsList)
            {
                ...
                kwsNode.Kws.Sm.RequestTaskSwitch(KwsTask.Remove);
            }
        ...
        /// <summary>
        /// Return a set containing the email addresses of all the users in all the
        /// workspaces.
        /// </summary>
        public SortedSet GetKwsEmailAddressSet()
        {
            return GetKwsEmailAddressSet(-1);
        }

        /// <summary>
        /// Return a set containing the topN most used email address of users in all
        /// the workspaces. The returned set contains the topN item sorted in alphabetical
        /// order.
        /// </summary>
        public SortedSet GetKwsEmailAddressSet(int topN)
        {
            Dictionary<string, int> emailFreq = new Dictionary<string,int>();
            SortedList<int, string> emailTops = new SortedList<int, string>();
            ...
            foreach (Workspace kws in m_wm.KwsTree.Values)
            {
                ...
                {
                    // If we want just a certain number of the most frequently
                    // occuring email addresses, then we need to count them.
                    if (topN >= 0)
                    {
                        if (emailFreq.ContainsKey(user.EmailAddress))
                        {
                            emailFreq[user.EmailAddress]++;
                        }
                        else
                        {
                            emailFreq.Add(user.EmailAddress, 1);
                        }
                    }
                    // If we want everything there is, don't bother counting.
                    else set.Add(user.EmailAddress);
                }
            }

            if (topN >= 0)
            {
                // If the number of items we found is under the number
                // of email addresses we demanded, just return the whole
                // set.
                if (topN >= emailFreq.Count)
                {
                    set.AddAll(emailFreq.Keys);
                }
                // If the number of items we found is higher than what we
                // demanded, then we select the most frequent.
                else if (topN < emailFreq.Count)
                {
                    int[] freqAr = new int[emailFreq.Count];
                    string[] valAr = new string[emailFreq.Count];

                    emailFreq.Keys.CopyTo(valAr, 0);
                    emailFreq.Values.CopyTo(freqAr, 0);

                    Array.Sort(freqAr, valAr);
                    Array.Reverse(valAr);

                    for (int i = 0; i < topN; i++) set.Add(valAr[i]);
                }
            }
    ...
    public class KwsBrowserKwsNode : KwsBrowserNode
    {
        ...
        public Color GetKwsIconImageKey()
        {
            ...
            if (curTask == KwsTask.Spawn ||
                curTask == KwsTask.Remove ||
                curTask == KwsTask.Rebuild)
            {
                return Color.Gray;
            }

            else if (curTask == KwsTask.Stop || curTask == KwsTask.WorkOffline)
            {
                return Color.Red;
            }
